...
public class RotationPointer : Object {
    ...
    public static RotationPointer? unmarshal(uint8[] b) {
        ...
        uint8[]? old_bytes = read_field(b, ref off);
        if (old_bytes == null) return null;
        uint8[]? new_bytes = read_field(b, ref off);
        if (new_bytes == null) return null;
        if (off + 8 > b.length) return null;
        p.old_aik = (!) old_bytes;
        p.new_aik = (!) new_bytes;
        p.rotated_at = (int64) get_u64(b, ref off);

        uint8[]? reason_bytes = read_field(b, ref off);
        if (reason_bytes == null) return null;
        if (((!) reason_bytes).length > MAX_REASON_BYTES) return null;
        var sb = new StringBuilder();
        foreach (uint8 c in (!) reason_bytes) sb.append_c((char) c);
        p.reason = sb.str;

        uint8[]? sig_ed = read_field(b, ref off);
        if (sig_ed == null) return null;
        uint8[]? sig_ml = read_field(b, ref off);
        if (sig_ml == null) return null;
    ...
    private static uint8[]? read_field(uint8[] b, ref int off) {
        if (off + 2 > b.length) return null;
        int len = (int) get_u16(b, ref off);
        if ((int64) off + (int64) len > (int64) b.length) return null;
        uint8[] v = new uint8[len];
        if (len > 0) Memory.copy(v, (uint8*) b + off, len);
        off += len;
        return v;
    }
}
